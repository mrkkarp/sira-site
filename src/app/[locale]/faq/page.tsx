...
export async function generateMetadata({
  ...
  // Only locales with real content are indexable; the rest (en/pl, which have
  // no source prose) stay on the `noindex` placeholder metadata.
  if (!content) {
    return buildPlaceholderMetadata(locale, `/${SLUG}`, dictionary.pages.faq);
  }
  ...
  return {
    ...
    ...pageSeo({
      locale,
      path: `/${SLUG}`,
      title: `${dictionary.pages.faq} — ${dictionary.site.name}`,
      description,
...
export default async function Page({
  ...
  if (!content) {
    return (
      <PlaceholderPage title={dictionary.pages.faq} dictionary={dictionary} />
    );
  }
  return (
    <InfoPage title={dictionary.pages.faq} content={content} locale={locale} />
  );
}
