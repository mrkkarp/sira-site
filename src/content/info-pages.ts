...
/**
 * Long-form INFO-page body content. `payment-delivery` and `returns` started as
 * a VERBATIM transcription of the live ODUDLAB (Horoshop) export in
 * `_horoshop-export/pages-html/`, and have since been corrected by the owner on
 * 2026-08-11 — additions marked in place ("Строки" on `payment-delivery`,
 * "Пошкодження та дефекти" on `returns`), and one removal: the archive's
 * 14-day exchange window on `returns`, which the owner struck out because
 * everything is made to order. Both edits exist because Google's
 ...
 * the other. Where the archive and the owner disagree, the owner wins and the
 * archive text goes. `care` is the one fully rewritten page — see its note below.
      ...
      sections: [
        {
          paragraphs: [
            ...
            // Owner, 2026-08-11, дослівно, на питання чи можна повернути виріб
            // належної якості: «немає. бо вироби виготовляються під
            // замовлення». Стара Horoshop-редакція обіцяла тут 14 днів на
            // обмін — вона суперечила і цій відповіді, і
            // `MerchantReturnNotPermitted` у структурованих даних товару, тож
            // власник розпорядився її прибрати (2026-08-11: «прибери»). Разом
            // із нею пішли й похідні від неї абзаци: перелік «випадків, коли
            // товар не підлягає обміну» (він має сенс лише там, де обмін
            // взагалі можливий) і опис повернення коштів при відмові від
            // товару. Якщо політика колись зміниться, міняти це речення і
            // `hasMerchantReturnPolicy` у `src/lib/product-structured-data.ts`
            // одночасно — інакше сторінка й розмітка знову розійдуться.
            "Кожен виріб ODUDLAB виготовляється під замовлення, тому повернення або обмін виробу належної якості не передбачені.",
        ...
        },
        {
          ...
          // Страхування на повну вартість і вимога фотоматеріалів — з архівної
          // Horoshop-редакції; вони пережили зняття «14 днів», бо стосуються
          // саме пошкоджень і дефектів, а не добровільної відмови.
          heading: "Пошкодження та дефекти",
          paragraphs: [
            "Усі наші посилки застраховані на повну вартість. Оглядайте виріб одразу у відділенні «Нової пошти», ще до того, як заберете його: якщо пошкодження виявлено вже після отримання, пошта страховку не виплачує — і відшкодувати збиток стає неможливо.",
            "Якщо ж дефект виробничий, це наша відповідальність: зворотну доставку або утилізацію виробу оплачує виробник.",
            "У будь-якому з цих випадків надішліть нам фотографії пошкодження або дефекту — без них розглянути звернення неможливо.",
          ],
        },
      ],
    },
    ...
    uk: {
      sections: [
        ...
        },
        {
          ...
          paragraphs: [
            // Той самий факт, що й на /returns, і в `hasMerchantReturnPolicy`:
            // повернення виробу належної якості немає, бо все виготовляється
            // під замовлення (owner, 2026-08-11). Тримати тут стару відповідь
            // про 14 днів означало б показувати покупцю дві різні політики на
            // двох сторінках одного сайту.
            "Гарантія на вироби — 12 місяців. Вироби виготовляються під замовлення, тому повернення чи обмін виробу належної якості не передбачені: за постановою Кабміну №172 індивідуальні замовлення обміну й поверненню не підлягають. Якщо ж виріб приїхав пошкодженим або має виробничий дефект — напишіть нам одразу, це ми беремо на себе.",
          ],
          link: { href: "/returns", label: "Гарантія, пошкодження та дефекти" },
        },
        {
